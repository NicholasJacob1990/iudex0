...
MODEL_NAME = "google/gemini-2.0-flash-001"  # Gemini 2.0 Flash via OpenRouter
MAX_CHARS_PER_CHUNK = 40_000  # ~10k tokens, conservador
...
- Utilize formatação e estrutura com parágrafos bem definidos, facilitando a leitura e compreensão. Evite parágrafos longos
...
- Corrija vícios de linguagem, como repetições desnecessárias, uso excessivo de advérbios, linguagem vaga ou imprecisa, gírias, expressões redundantes
- NÃO mencione os falantes (SPEAKER, Professor:, Aluno:, etc.) - converta para texto impessoal didático
- Divida a aula em tópicos e subtópicos para melhor organização
- Enumere os tópicos sequencialmente (1, 1.1, 1.2, 2, 2.1, etc.), use negrito
- Intercale parágrafos curtos e longos, tornando a leitura menos cansativa
- Seja didático sem perder detalhes e conteúdo
- USE TEXTO CORRIDO NA MEDIDA DO POSSÍVEL
- Ao final de cada tópico principal, sintetize o assunto em tabela
...
def processar_com_openrouter(texto: str, parte_num: int = 1, total_partes: int = 1, ultimo_topico: int = 0) -> str:
    """Processa um chunk de texto com Gemini via OpenRouter"""
    
    # Contexto para manter numeração contínua
    if parte_num == 1:
        contexto_numeracao = "\n- Esta é a PRIMEIRA parte. Inicie a numeração dos tópicos em 1."
    else:
        contexto_numeracao = f"\n- Esta é a PARTE {parte_num} de {total_partes}. Continue a numeração a partir do tópico {ultimo_topico + 1}. NÃO reinicie a numeração."
    ...
    try:
        ...
        print(f"{Fore.RED}⚠️  Erro na requisição: {e}")
        if hasattr(e, 'response') and e.response:
            print(f"{Fore.RED}   Resposta: {e.response.text}")
        raise
...
def main():
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            print(f"{Fore.GREEN}✅ Parte {i} processada (último tópico: {ultimo_topico})")
            
            if i < len(chunks):
                print(f"{Fore.CYAN}⏳ Aguardando 1 segundo...")
                time.sleep(1)
