"""
GraphRAG / ArgumentRAG Module for Iudex MVP Fast
...
3. Persistence by scope (private/group/global)
...
DEFAULT_GRAPH_DB_PATH = os.path.join(os.path.dirname(__file__), "graph_db")
DEFAULT_TOKEN_BUDGET = 2000  # tokens for graph context
...
class LegalEntityExtractor:
    ...
    PATTERNS = {
        ...
        "jurisprudencia": re.compile(
            r"(RE|REsp|AgRg|ADI|ADC|ADPF|HC|MS|RMS|RO|AI|ARE|Rcl)\s*n?[o]?\s*([\d.]+)(?:/([\w]{2}))?",
            re.IGNORECASE,
        ),
        "tema": re.compile(
...
class LegalKnowledgeGraph:
    ...
    def __init__(
        ...
        if persist_path:
            ...
            base_dir = DEFAULT_GRAPH_DB_PATH
    ...
    def query_related(
        ...
        with self._lock:
            if node_id not in self.graph.nodes:
                return {"nodes": [], "edges": [], "center": node_id}
    ...
    def build_context(
        ...
        for entity_id in entity_ids:
            ...
            subgraph = self.query_related(entity_id, hops=hops, max_nodes=20)
    ...
    def query_context_from_text(
        ...
        if not entity_ids:
            return "", []
        return self.build_context(entity_ids, hops, token_budget), list(entity_ids)
    ...
    def enrich_chunks(
        ...
        return self.build_context(extracted_entities, hops, token_budget)
    ...
    def ingest_text(
        ...
        if self.pack and added_entities:
            ...
            for src, tgt, rel_type, meta in relations:
                if self.add_relationship(src, tgt, rel_type, metadata=meta):
