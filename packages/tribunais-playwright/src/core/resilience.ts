...
import type { ResilienceConfig } from '../types/index.js';
...
export function classifyError(error: unknown): ErrorKind {
  ...
  // Erros de timeout e rede são transitórios
  if (
...
/**
 * Executa função com timeout curto.
 * Se a função não resolver dentro do timeout, rejeita imediatamente.
 */
...
export interface RetryOptions {
  maxRetries: number;
  backoff: number;
  /** Só faz retry para estes tipos de erro */
  retryOn?: ErrorKind[];
}

/**
 * Executa função com retry e exponential backoff.
 * Por padrão, só faz retry para erros transitórios.
 */
