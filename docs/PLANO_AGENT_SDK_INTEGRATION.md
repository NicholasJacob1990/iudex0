...
*Atualizado em 2026-02-07: implementação inicial da Fase 4.1/4.3/4.5 (feature flags em camadas, circuit breaker no MCPHub, quotas por tenant) com testes unitários dedicados (`test_feature_flags.py`, `test_mcp_circuit_breaker.py`, `test_user_quotas.py`).*
*Atualizado em 2026-02-07: implementação da Fase 4.4 (auditoria exportável) com `observability/audit_log.py`, integração em PermissionManager/executors e endpoints de export (`/audit/tool-calls`, `/audit/tool-calls/export`).*
*Atualizado em 2026-02-07: implementação da Fase 4.6 (contratos operacionais MCP) com `shared/mcp_contracts.py` (ACL/rate-limit/cache/segredos por tenant), integração no `MCPHub` e microserviço `apps/mcp-legal-server/` (tools/list + tools/call), coberto por `test_mcp_contracts.py`.*
*Atualizado em 2026-02-07: Skill Builder v1 reforçado para público básico/avançado com validações cruzadas de schema (`version` semver, `audience`, `prefer_workflow` vs `prefer_agent`, overlap de tools), métricas de qualidade no `/skills/validate` (`quality_score`, `tpr`, `fpr`, `security_violations`, `improvements`) e UX de público-alvo/métricas na UI (`skill-wizard.tsx`, `skill-editor.tsx`).*
*Atualizado em 2026-02-07: smoke test E2E do endpoint MCP standalone (`apps/mcp-legal-server`) executado com conectores reais DataJud/DJEN via `tools/call`, com respostas `success=true` para `consultar_processo_datajud` e `buscar_publicacoes_djen` (total 0 no NPU de teste), validando caminho fim-a-fim sem erro de autenticação/conector.*
