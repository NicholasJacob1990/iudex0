...
- Integration with RAG config singleton
- LLM-based compression (extractive, abstractive, hybrid)
- Legal citation preservation (Art., Lei, Sumula, RE/AgRg/ADI)
"""
...
class CompressionConfig:
    ...
    @classmethod
    def from_rag_config(cls) -> "CompressionConfig":
        """Load configuration from RAG config singleton."""
        rag_cfg = get_rag_config()
        return cls(
            max_chars_per_chunk=rag_cfg.compression_max_chars,
            token_budget=rag_cfg.compression_token_budget,
            chars_per_token=4.0,
            min_keyword_length=4,
            preserve_full_text=rag_cfg.compression_preserve_full_text,
            min_sentences=1,
            max_sentences_to_evaluate=50,
...
class ContextCompressor:
    ...
    def __init__(self, config: Optional[CompressionConfig] = None):
        self.config = config or CompressionConfig.from_rag_config()
...
class TokenBudgetManager:
    ...
    @classmethod
    def from_rag_config(cls) -> "TokenBudgetManager":
        """Load configuration from RAG config singleton."""
        rag_cfg = get_rag_config()
        return cls(
            total_budget=rag_cfg.compression_token_budget * 2,  # Context budget is typically larger
            chars_per_token=4.0,
            rag_share=0.6,
            graph_share=0.25,
            reserved_share=0.15,
