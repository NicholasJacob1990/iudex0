...
class ExpansionConfig:
    ...
    @classmethod
    def from_rag_config(cls) -> "ExpansionConfig":
        """Load configuration from RAG config singleton."""
        rag_config = get_rag_config()
        return cls(
            window=rag_config.chunk_expansion_window,
            max_extra_per_hit=rag_config.chunk_expansion_max_extra // 2,  # Split between per-hit and total
            max_extra_total=rag_config.chunk_expansion_max_extra,
            preserve_order=True,
            merge_adjacent=rag_config.chunk_expansion_merge_adjacent,
            merge_separator="\n\n",
            max_total_chars=0,
        )
...
class ChunkExpander:
    ...
    def __init__(
        ...
        Args:
            config: Expansion configuration. If None, loads from RAG config.
            chunk_fetcher: Function(doc_id, chunk_index) -> chunk dict or None
        """
        self.config = config or ExpansionConfig.from_rag_config()
...
def create_expander_from_rag_config(
    qdrant_service: Optional[Any] = None,
    opensearch_service: Optional[Any] = None,
) -> ChunkExpander:
    """
    Create a ChunkExpander using the RAG config singleton.

    This factory uses the centralized RAG configuration to determine
    settings and optionally wire up storage backends.

    Args:
        qdrant_service: Optional QdrantService for chunk fetching
        opensearch_service: Optional OpenSearchService for chunk fetching
    ...
    # Check if chunk expansion is enabled
    if not rag_config.enable_chunk_expansion:
        logger.info("Chunk expansion is disabled in RAG config")
        # Return a no-op expander
        config = ExpansionConfig(
            window=0,
            max_extra_per_hit=0,
            max_extra_total=0,
        )
        return ChunkExpander(config)

    # Create config from RAG settings
    config = ExpansionConfig.from_rag_config()

    # Wire up fetcher if service provided
    if qdrant_service is not None:
        return create_expander_with_qdrant(
            qdrant_service,
            rag_config.qdrant_collection_local,
            config,
        )
    elif opensearch_service is not None:
        return create_expander_with_opensearch(
            opensearch_service,
            rag_config.opensearch_index_local,
            config,
        )

    # Return expander without fetcher (caller can set it later)
    return ChunkExpander(config)
