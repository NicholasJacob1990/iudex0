...
- LangSmith export option
- Event-based tracing for all RAG pipeline stages

Configuration is loaded from the RAG config singleton, which reads from
environment variables. See app.services.rag.config for all available settings.
"""
...
class TraceEventType(str, Enum):
    ...
    QUERY_REWRITE = "QUERY_REWRITE"  # Generic query rewrite
    # Error events
    ERROR_OCCURRED = "ERROR_OCCURRED"
    # Ingestion events
    INGEST_STARTED = "INGEST_STARTED"
    INGEST_COMPLETE = "INGEST_COMPLETE"
    # Chunk expansion events
    CHUNK_EXPANSION_COMPLETE = "CHUNK_EXPANSION_COMPLETE"
    PARENT_CHILD_EXPAND = "PARENT_CHILD_EXPAND"
    # Lexical-first gating events
    LEXICAL_FIRST_GATE = "LEXICAL_FIRST_GATE"
    # Legacy events (for backward compatibility with rag_trace.py)
    HYDE_GENERATE = "HYDE_GENERATE"
    GRAPH_EXPAND = "GRAPH_EXPAND"
    ARGUMENT_CONTEXT = "ARGUMENT_CONTEXT"
    CONTEXT_COMPRESS = "CONTEXT_COMPRESS"
    FALLBACK = "FALLBACK"
    RAG_ROUTER_DECISION = "RAG_ROUTER_DECISION"
    PROMPT_FINAL = "PROMPT_FINAL"
    # Generic event for unmapped legacy events
    GENERIC = "GENERIC"
...
def _write_jsonl(record: Dict[str, Any]) -> None:
    ...
    if not config.enable_tracing:
        return
    try:
        log_path = config.trace_log_path
...
def _emit_otel(event_type: str, payload: Dict[str, Any]) -> None:
    ...
    config = get_rag_config()
    if not config.trace_export_otel:
        return
    try:
...
def _emit_langsmith(event_type: str, payload: Dict[str, Any]) -> None:
    ...
    config = get_rag_config()
    if not config.trace_export_langsmith:
        return
    try:
        ...
        client.create_run(
            ...
            run_type="tool",
            inputs={
                "query": payload.get("query_original"),
...
async def _persist_db_async(record: Dict[str, Any]) -> None:
    ...
    config = get_rag_config()
    if not config.trace_persist_db:
        return
    try:
...
# ---------------------------------------------------------------------------
# Async Context Manager for Timed Traces
# ---------------------------------------------------------------------------

class AsyncTraceTimer:
    """
    Async context manager for timing RAG operations and automatically recording trace events.

    Usage:
        async with AsyncTraceTimer(TraceEventType.VECTOR_SEARCH_COMPLETE, request_id="abc") as timer:
    ...
    async def __aenter__(self) -> "AsyncTraceTimer":
        self._start_time = time.perf_counter()
        return self
...
def trace_crag_gate(
    ...
    """Convenience function to trace CRAG gate evaluation."""
    return trace_event(
        TraceEventType.CRAG_GATE_EVALUATED,
        request_id=request_id,
        query_original=query,
        evidence_level=best_score,
...
def trace_rerank_complete(
    ...
    """Convenience function to trace reranking completion."""
    return trace_event(
...
def trace_compression_applied(
    ...
    """Convenience function to trace compression."""
    return trace_event(
...
def trace_response_sent(
    ...
    """Convenience function to trace response completion."""
    return trace_event(
        ...
        metadata={"chunks_returned": chunks_returned},
    )


# ---------------------------------------------------------------------------
# Tracing Status Helper
# ---------------------------------------------------------------------------
...
def get_trace_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "log_path": config.trace_log_path,
        "persist_db": config.trace_persist_db,
        "export_otel": config.trace_export_otel,
        "export_langsmith": config.trace_export_langsmith,
    }


# ---------------------------------------------------------------------------
# Legacy Compatibility Layer (for migration from rag_trace.py)
# ---------------------------------------------------------------------------

# Mapping from legacy string events to TraceEventType
_LEGACY_EVENT_MAP: Dict[str, TraceEventType] = {
    "query_rewrite": TraceEventType.QUERY_REWRITE,
    "hyde_generate": TraceEventType.HYDE_GENERATE,
    "graph_expand": TraceEventType.GRAPH_EXPAND,
    "argument_context": TraceEventType.ARGUMENT_CONTEXT,
    "fallback": TraceEventType.FALLBACK,
    "parent_child_expand": TraceEventType.PARENT_CHILD_EXPAND,
    "context_compress": TraceEventType.CONTEXT_COMPRESS,
    "graph_enrich": TraceEventType.GRAPH_ENRICHED,
    "rag_router_decision": TraceEventType.RAG_ROUTER_DECISION,
    "prompt_final": TraceEventType.PROMPT_FINAL,
}
...
def trace_event_legacy(
    ...
    *,
    ...
    """
    Legacy-compatible trace_event function.

    This function provides backward compatibility with the old rag_trace.py API.
    It maps string event names to TraceEventType enums and delegates to the new
    trace_event function.

    Args:
        event: String event name (legacy format)
        payload: Dict payload with event-specific data
        request_id: Request identifier
        user_id: User identifier
        tenant_id: Tenant identifier
        conversation_id: Conversation identifier (for chat context)
        message_id: Message identifier (for chat context)

    Example:
        # Old usage (rag_trace.py):
        trace_event("query_rewrite", {"original": q, "rewritten": rq}, request_id=rid)

        # New usage (this function):
        trace_event_legacy("query_rewrite", {"original": q, "rewritten": rq}, request_id=rid)
    """
    # Map legacy event string to enum, defaulting to GENERIC
    event_type = _LEGACY_EVENT_MAP.get(event.lower(), TraceEventType.GENERIC)

    # Extract known fields from payload
    query_original = payload.get("original") or payload.get("query")
    query_rewritten = payload.get("rewritten")

    # Call the new trace_event with mapped values
    trace_event(
        event_type,
        request_id=request_id,
        query_original=query_original,
